...
Render (high quality -- this deck is displayed at half-slide width via
Slidev's iframe-right layout, so it needs the extra resolution):
    manim-slides render -q h chao_das_derivation.py ChaoDasDerivation
Present standalone (keystroke-driven, arrow keys / space to advance):
    manim-slides present ChaoDasDerivation
Export to a self-contained offline HTML player, for embedding in Slidev
via iframe (see ../slides.md's "The Model" slide and ../README.md):
    manim-slides convert ChaoDasDerivation ../public/chao_das_derivation.html \
        --offline -cembedded=true -cdisable_layout=true \
        --use-template embedded_template.html

Every flag above is load-bearing, not cosmetic:
  --offline           no CDN dependency (reveal.js/CSS bundled locally) --
                       the default online export renders blank with no
                       internet access.
  -cembedded=true      tells reveal.js it's confined to part of a page
                       rather than owning the whole viewport.
  -cdisable_layout=true + embedded_template.html's explicit CSS: reveal.js's
                       own auto-scale/resize handling goes into an infinite
                       Y->X->Zt mutual-recursion loop (RangeError: Maximum
                       call stack size exceeded) once nested inside Slidev's
                       iframe-right layout -- confirmed by rendering: it
                       never happens standalone, only once embedded in a
                       flex/grid-sized iframe rather than the raw browser
                       viewport. Neither `embedded=true` alone nor fixed
                       pixel width/height alone fixed it; only disabling
                       reveal's own layout did. That in turn drops the CSS
                       reveal.js would have set to make the deck actually
                       fill its container, which embedded_template.html
                       (a copy of manim-slides' default HTML template plus
                       one added <style> block) restores by hand.
  Do NOT use --one-file: it corrupts the embedded video streams (found
  while first building this section) -- --offline without --one-file
  keeps each animation as its own small mp4 under a local assets folder,
  which plays fine.
