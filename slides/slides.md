...
Full derivation, animated with Manim
</div>

<div class="text-sm pt-6" style="color:#c2410c">
▸ click into the panel, then press space / → to advance — one equation transforms into the next, one keystroke at a time
</div>
...
and embedded here. Click into the panel and use arrow keys / space to step
through it — each step is its own keystroke, matching the rest of the deck's
click-to-advance rhythm.
...
2. From two phases to one Φ-equation: solid-phase transport (Eq. 5) →
   ...
   session's dimensional bug lived — flagged on-screen, not just in the repo.
...
Deliberately kept compact and non-verbose on-screen — a caption of a few
...
Rendered offline (manim-slides convert --offline, not --one-file, which
was found to corrupt embedded video streams): all assets are local, no
CDN dependency, one small mp4 per animation step under
public/chao_das_derivation_assets/.
-->
